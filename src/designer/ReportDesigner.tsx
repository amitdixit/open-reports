...
const ReportDesigner = () => {
    ...
    const createInitialBands = (): BandModel[] => {
        ...
        const baseBands: { id: string; type: BandType; items: ReportItemModel[] }[] =
            [
    ...
    const alignSelectedItem = (
        ...
        if (selected.length === 1) {
            ...
            const contentWidth =
                report.page.width -
                report.page.margin.left -
                report.page.margin.right;

            if (mode === "left") x = 0;
            if (mode === "center")
                x = Math.round((contentWidth - item.width) / 2);
            if (mode === "right") x = contentWidth - item.width;

            if (mode === "top") y = 0;
            if (mode === "middle")
                y = Math.round((band.height - item.height) / 2);
        ...
        const maxX = Math.max(
            ...selected.map(s => s.item.x + s.item.width)
        );
        const minY = Math.min(...selected.map(s => s.item.y));
        const maxY = Math.max(
            ...selected.map(s => s.item.y + s.item.height)
        );
